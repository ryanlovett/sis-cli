...
# SIS endpoint
enrollments_uri = "https://gateway.api.berkeley.edu/sis/v3/enrollments"
...
async def get_lecture_section_ids(
    app_id, app_key, term_id, subject_area, catalog_number=None
):
    """
    ...
    headers = {"Accept": "application/json", "app_id": app_id, "app_key": app_key}
    params = {"page-number": 1, "subject-area-code": subject_area}
    if catalog_number:
    ...
    sections = await sis.get_items(uri, params, headers, "fieldValueList.fieldValues")
...
async def get_enrollments(app_id, app_key, term_id, subject_area, catalog_number):
    ...
    # get the lectures
    lecture_codes = await get_lecture_section_ids(
    ...
    for section_id in lecture_codes:
        enrollments += await get_section_enrollments(
            app_id, app_key, term_id, section_id
        )
